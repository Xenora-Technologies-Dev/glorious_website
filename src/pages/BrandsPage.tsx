  ...
  return (
    <>
      ...
      <PageHero
        ...
        copy="Owned and private-label brands as they appear in the company catalogue. Atmospheric imagery is not product packaging."
        crumbs={[
          { label: 'Home', href: '/' },
          { label: 'Brands' },
        ]}
          ...
          <div className="relative aspect-[16/10] overflow-hidden lg:hidden">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6">
              <p className="text-[11px] tracking-[0.2em] uppercase text-gold">
                {current.hasPortfolio ? 'Catalogue brand' : 'Artwork only'}
              </p>
              <p className="mt-2 font-display text-3xl">{current.name}</p>
            </div>
          </div>
          <div className="flex flex-col justify-center lg:col-span-5 lg:min-h-[70svh]">
            {brands.map((brand) => (
              <Link
                ...
                className="group border-b border-line-light py-5 first:border-t"
              >
                <span
                  className={
                    active === brand.slug
                      ? 'font-display text-[2rem] text-gold sm:text-5xl'
                      : 'font-display text-[2rem] text-ivory/55 transition-colors duration-500 group-hover:text-ivory sm:text-5xl'
                  }
                >
                  {brand.name}
                </span>
